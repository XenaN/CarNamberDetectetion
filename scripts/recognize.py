...
def save_cropp_image(image: PathLike, num_plate: List, count: int, output_path: PathLike) -> PathLike:
    """
...
def reсognize_letters(image: PathLike, output_path: PathLike):
    """
    ...
    for num_plate in final_result:
        count += 1
        save_path = save_cropp_image(image, num_plate, count, output_path)
        
        custom_config = r"-c tessedit_char_whitelist=0123456789ABEKMHOPCTYX --oem 3 --psm 6"

        plate = pytesseract.image_to_string(save_path, config=custom_config)
